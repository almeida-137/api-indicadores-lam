...
FROM
    ...
    (select SUM(measured_weight)/1000 AS ATUAL_HORA_PESO from fladc04 where to_timestamp(to_char(discharge_time, 'YYYY-MM-DD HH24:MI:SS'), 'YYYY-MM-DD HH24:MI:SS') > to_timestamp(to_char((systimestamp - extract(minute from systimestamp)/24/60 - extract(second from systimestamp)/24/60/60),'YYYY-MM-DD hh24:mi:ss'),'YYYY-MM-DD HH24:MI:SS')),
    (select SUM(measured_weight)/1000 AS ULTIMA_HORA_PESO from fladc04 where to_timestamp(to_char(discharge_time, 'YYYY-MM-DD HH24:MI:SS'), 'YYYY-MM-DD HH24:MI:SS') between to_timestamp(to_char((systimestamp - extract(minute from systimestamp)/24/60 - extract(second from systimestamp)/24/60/60 -1/24+1/24/60/60),'YYYY-MM-DD hh24:mi:ss'),'YYYY-MM-DD HH24:MI:SS') and to_timestamp(to_char((systimestamp - extract(minute from systimestamp)/24/60 - extract(second from systimestamp)/24/60/60),'YYYY-MM-DD hh24:mi:ss'),'YYYY-MM-DD HH24:MI:SS')),
    (select sum(measured_weight)/1000 as PESO_TOTAL from fladc04 where to_char(discharge_time, 'YYYY-MM-DD') = to_char(sysdate,'YYYY-MM-DD')),
